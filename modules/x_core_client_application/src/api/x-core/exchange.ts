import {CONFIG} from "@Api/x-core";
import {ERequestMethod} from "@Lib/api/types";

export const getHeaders = CONFIG.getDefaultHeaders;

export const getServerUrl = CONFIG.getServerUrl;
...
const doRequest = async (method: ERequestMethod, mapping: string, request?: object | URLSearchParams, headers?: Headers): Promise<object> => {
  ...
  try {
    rawResponse = await fetch(getServerUrl() + mapping, rawRequest);
