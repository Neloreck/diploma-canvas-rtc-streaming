...
import com.xcore.server.controllers.rest.exchange.{ApiResponse, FailedApiResponse}
...
class ProfileController {
  ...
  case class ProfileNotFoundResponse(id: Long) extends FailedApiResponse(s"Profile with id $id was not found.");
  ...
  def getAppProfile(@PathVariable(value="id") id: String): ResponseEntity[ApiResponse] = {
    ...
    try {
      ...
      case exception: Exception =>
        return new ResponseEntity[ApiResponse](new FailedApiResponse("Bad profile id supplied: $id."), HttpStatus.BAD_REQUEST);
    }
